package ekh.control;

import java.io.IOException;
import java.sql.SQLException;

import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import ekh.bean.AmministratoreBean;
import ekh.bean.ClienteBean;
import ekh.bean.PacchettoBean;
import ekh.bean.ParametroBean;
import ekh.model.ParametroModelDM;

@WebServlet("/PacchettoControlServlet")
public class PacchettoControlServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	ParametroModelDM model = new ParametroModelDM();

	public PacchettoControlServlet() {
		super();
	}

	protected void doGet(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		String redirectedPage = "/jsp/HomePage.jsp";

		Boolean adminRoles = (Boolean) request.getSession().getAttribute("adminRoles");
		AmministratoreBean admin = (AmministratoreBean) request.getSession().getAttribute("Admin");
		Boolean userRoles = (Boolean) request.getSession().getAttribute("userRoles");
		ClienteBean user = (ClienteBean) request.getSession().getAttribute("User");

		if ((admin == null || adminRoles == null || !adminRoles.booleanValue())
				&& (user == null || userRoles == null || !userRoles.booleanValue())) {
			redirectedPage = "/jsp/HomePage.jsp";
		} else {
			PacchettoBean pacchettoTemp = (PacchettoBean) request.getSession().getAttribute("pacchettoTemp");
			if (pacchettoTemp == null) {
				redirectedPage = "/jsp/CreaPacchetto.jsp";
			} else {
				String action = request.getParameter("action");
				try {
					if (action != null) {
						if (action.equals("addParametro")) {
							ParametroBean bean = new ParametroBean();
							String id = request.getParameter("id");
							try {
								if (id != null) {
									try {
										bean = model.doRetrieveByKey(id);
										if (!bean.isEmpty() && bean != null) {
											pacchettoTemp.addParametro(bean);
										} else
											throw new Exception("ERRORE - PacchettoControlServlet: parametro inesistente.");
									} catch (SQLException e) {
										System.out.println(e.getMessage());
									}
								} else
									throw new Exception("ERRORE-PacchettoControlServlet-addParametro: id vuoto o null.");
							} catch (Exception e) {
								System.out.println(e.getMessage());
							}
						} else if (action.equals("remParametro")) {
							ParametroBean bean = new ParametroBean();
							String id = request.getParameter("id");
							try {
								if (id != null) {
									try {
										bean = model.doRetrieveByKey(id);
										if (!bean.isEmpty() && bean != null) {
											pacchettoTemp.remParametro(bean);
										} else
											throw new Exception("ERRORE - PacchettoControlServlet: parametro inesistente.");
									} catch (SQLException e) {
										System.out.println(e.getMessage());
									}
								} else
									throw new Exception("ERRORE-PacchettoControlServlet-addParametro: id vuoto o null.");
							} catch (Exception e) {
								System.out.println(e.getMessage());
							}
						} else if (action.equals("svuota")) {
							pacchettoTemp.deleteContenuto();
						} else
							throw new Exception("ERRORE - VisualizzaParametriServlet: action vuota o null.");
					} else
						throw new Exception("ERRORE - VisualizzaParametriServlet: action vuota o null.");
				} catch (Exception e) {
					System.out.println(e.getMessage());
				}
				
				request.getSession().removeAttribute("pacchettoTemp");
				request.getSession().setAttribute("pacchettoTemp", pacchettoTemp);
			}
		}
		redirectedPage = response.encodeURL(redirectedPage);
		RequestDispatcher dispatcher = this.getServletContext().getRequestDispatcher(redirectedPage);
		dispatcher.forward(request, response);
	}

	protected void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		doGet(request, response);
	}

}
